...
int binarioParaDecimal(int binario, int decimal, int peso)
{
    if(binario <= 0){
        return decimal;
    }
    return binarioParaDecimal(binario / 10, decimal + (binario % 10) * peso, peso * 2);
}
...
int main()
{
    ...
    int decimal = 0;
    int peso = 1;
    decimal = binarioParaDecimal(binario, decimal, peso);
